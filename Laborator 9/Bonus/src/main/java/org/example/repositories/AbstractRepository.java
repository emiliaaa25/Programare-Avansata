...
public abstract class AbstractRepository<T> {
    ...
    public List<T> findAll() {
        ...
        try {
            ...
            logger.log(Level.INFO, "Found entities: {0}", entities);
            return entities;
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Error while finding all entities", e);
            throw e;
        } finally {
            em.close();
            long endTime = System.currentTimeMillis();
            logger.log(Level.INFO, "Execution time for findAll method: {0} ms", (endTime - startTime));
            logger.log(Level.INFO, "EntityManager closed after finding all entities");
        }
    }
}
